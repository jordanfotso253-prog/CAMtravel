...
async function camtravelGetOccupiedSeats(tripId, travelDate) {
  ...
  try {
    const { data, error } = await window.camtravelSupabase
      .from('reservations').select('seat_numbers')
      .eq('trip_id', tripId).eq('travel_date', travelDate)
      .not('seat_numbers', 'is', null);
    if (error) throw error;
    const occupied = [];
    (data || []).forEach(r => {
...
async function camtravelRequestRefund(reservationId, reason) {
  ...
  try {
    const { error } = await window.camtravelSupabase
      .from('reservations')
      .update({ refund_status: 'requested', refund_reason: reason || null, refund_requested_at: new Date().toISOString() })
      .eq('id', reservationId);
    if (error) throw error;
    return true;
  } catch (e) {
